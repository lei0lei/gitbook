...
## 变量绑定 <a href="#bian-liang-bang-ding" id="bian-liang-bang-ding"></a>
...
变量绑定有一个作用域（scope），它被限定只在一个**代码块**（block）中生存（live）。 代码块是一个被 `{}` 包围的语句集合。另外也允许[变量遮蔽](https://en.wikipedia.org/wiki/Variable\_shadowing)（variable shadowing）。
